...
"""All the base classes and protocols describing QD algorithms (optimisers). All of these algorithms perform optimisation on a defined objective functions,
and use a provided container to store the evaluated solutions. The base abstract class of algorithms is `QDAlgorithm`,
itself implementing the `QDAlgorithmLike` Protocol. Most QD algorithms will subclass QDAlgorithm directly, but in some special cases,
algorithms might need to subclass QDAlgorithmLike instead (e.g. sequences `Sq` of QD algorithms).

All algorithms must be registered to the default qdpy.base.Registry `qdpy.base.registry` if they are to be easily accessed or created by factories.
It can be done by decorating QD algorithms classes with `@registry.register`. """
...
class QDAlgorithmLike(Protocol):
    """Protocol (Python interfaces) describing QD algorithms. All classes describing qdpy algorithms must implement
    this protocol -- or alternatively the `qdpy.algorithms.QDAlgorithm` abstract class, which already implements `qdpy.algorithms.QDAlgorithmLike`.
